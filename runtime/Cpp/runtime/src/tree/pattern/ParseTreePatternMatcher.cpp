...
ParseTreePattern ParseTreePatternMatcher::compile(const std::string &pattern, int patternRuleIndex) {
  ...
  try {
    ...
    std::throw_with_nested(antlr4::RuntimeException("Cannot invoke start rule")); // Wrap any other exception. We should however probably use one of the ANTLR exceptions here.
#endif
  }
